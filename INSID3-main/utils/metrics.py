...
class AverageMeter:
    ...
    def __init__(self, dataset_name: str, class_ids: list[int], device: str = "cuda"):
        ...
        self.intersection_buf = torch.zeros(2, self.nclass, device=self.device).float()
        self.union_buf = torch.zeros(2, self.nclass, device=self.device).float()
...
class Evaluator:
    ...
    def classify_prediction(cls, pred_mask: torch.Tensor, gt_mask: torch.Tensor, tgt_ignore_idx: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if tgt_ignore_idx is not None:
            ...
            # tgt_ignore_idx *= cls.ignore_index 更改了类型错误
            tgt_ignore_idx = tgt_ignore_idx.long() * cls.ignore_index
